...
shutters = [1/8000, 1/4000, 1/2000, 1/1000, 1/500, 1/250, 1/125, 1/60, 1/30, 1/15, 1/8, 1/2]
...
def compute_hdr(data):
    ...
    for i in range(len(data)):
        total += np.multiply(np.multiply(certainty[data[i]], np.power(data[i], np.divide(1,gamma))), 4 * shutters[i])
...
for i in range(numImages):
    imageNames.append("v" + str(i+1) + "_average_image.png")
